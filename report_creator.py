...
async def report_call(tg_id):
    ...
    # Проверка на класс
    if data_all[5] == 7: cur.execute("SELECT * FROM quiz_answers WHERE class_out >= 9 AND NOT class_out = 11")
    ...
    if data_all[5] == 10: cur.execute("SELECT * FROM quiz_answers WHERE class_out == 11")
    ...
    so_extract = []
    # Вывод 3 самых совпадающих строк (в зависимости от кол-ва данных, возможно и 5)!
    for row, matches in matching_counts[:3]:
...
async def report_callin(so_extract):
    so_extract_1 = so_extract[0]
    class_out_begin = so_extract_1[5]
    hw_time_begin = so_extract_1[6]
    general_ill_begin = so_extract_1[7]
    bbone_ill_begin = so_extract_1[8]
    neck_ill_begin = so_extract_1[9]
    vascular_ill_begin = so_extract_1[10]
    walk_begin = so_extract_1[11]
    hobby_begin = so_extract_1[12]
    return class_out_begin
